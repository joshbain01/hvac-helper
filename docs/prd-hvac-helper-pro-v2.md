...
graph LR
    ...
    C -->|HTTPS / TLS 1.3| D[Cloud Backend]
...
The cloud services provide secure synchronization:
*   **Gateway API**: Receives incoming JSON snapshot payloads, enforces upload idempotency, immediately queues payloads in an asynchronous message queue (e.g., AWS SQS or Redis bull-queue), and returns a `202 Accepted` response to mobile clients.
*   **CRM Integration**: Background workers process queued snapshots to perform database persistence and format/synchronize them with third-party Field Service Management (FSM) systems, handling outages and rate-limiting via exponential backoff retries and Dead Letter Queue (DLQ) containment.
*   **Authentication**: Secures API endpoints using short-lived JSON Web Tokens (JWT).
